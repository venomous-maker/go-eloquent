...
func (bc *BaseController[T]) Index(c *gin.Context) {
	...
	case "deleted":
		results, _, err = bc.Service.Query().OnlyTrashed().Paginate(pageInt, limitInt)
	case "all":
		results, _, err = bc.Service.Query().WithTrashed().Paginate(pageInt, limitInt)
	default:
...
func (bc *BaseController[T]) AddRoute(method, path string, handler gin.HandlerFunc, middleware ...gin.HandlerFunc) {
	bc.ExtraRoutes = append(bc.ExtraRoutes, GlobalRoutes.ExtraRoute{
...
//
//	userController.ExtraRoutes = append(userController.ExtraRoutes, GlobalRoutes.ExtraRoute{
//		Group: "/:userId/benefits",
//		Middleware: []gin.HandlerFunc{UserAuthMiddleware},
//		Children: []GlobalRoutes.ExtraRoute{
//			{
...
//   - HEAD
//   - OPTIONS
func (bc *BaseController[T]) RegisterRouteRecursive(group *gin.RouterGroup, route GlobalRoutes.ExtraRoute) {
